...
#if BARRET==1
static __inline __m128i mulmod_simd_epu32x4(__m128i av,__m128i bv,u32 mod,u32 mu_lo,u32 mu_hi){
...
static __inline void ntt1(u32*f,int n){
    ...
    for(int len=2;len<=n;len<<=1){
        ...
        for(int j=0;j<n;j+=len){
            ...
            for(;k+4<=half;k+=4){
                __m128i kvec=_mm_add_epi32(_mm_set1_epi32(k),idxbase);
                __m128i rootidx=_mm_mullo_epi32(kvec,stepvec);
                __m128i rootv=_mm_i32gather_epi32((const int*)root,rootidx,4);
                __m128i uvec=_mm_loadu_si128((__m128i*)&f[j+k]);
                __m128i vraw=_mm_loadu_si128((__m128i*)&f[j+k+half]);
